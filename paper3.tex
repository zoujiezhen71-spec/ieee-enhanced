...
than \$50 billion per year~\cite{Nandi2005,Randall2011}. Early detection
of incipient bearing faults before they cause secondary damage or
catastrophic failure is therefore a key industrial automation objective.
...
available~\cite{Guo2016,Shao2017,Lei2020}. However, these methods
...
neighbours, and neural networks. The LIBSVM library~\cite{Chang2011}
...
al.~\cite{Guo2016} achieve 99--100\% accuracy on the CWRU dataset
with raw waveform input, but require 10,000--50,000 training samples
per class and GPU inference hardware. In contrast, the proposed
RBF-SVM achieves \SI{98.0}{\percent} with only 280 samples per class
and runs on a standard CPU in under \SI{3}{\milli\second}. For
embedded industrial condition monitoring systems (e.g., PLCs,
microcontrollers) where data is scarce and GPU resources are
unavailable, the proposed approach is therefore more appropriate and
practically relevant~\cite{Liu2018,Widodo2007}.
...
full re-identification of the feature statistics.
